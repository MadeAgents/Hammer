...
OUT_DIR="../checkpoint/toolkit_qwen2_name"

deepspeed --num_gpus $num_gpus src/train.py \
    --deepspeed examples/deepspeed/ds_z2_config.json \
    --stage sft \
    --do_train \
    --model_name_or_path /home/notebook/data/group/model_hub/huggingface/Qwen/Qwen2-1.5B-Instruct \
    --dataset train2_seen \
    --output_dir $OUT_DIR \
    --template qwen \
    --dataset_dir ../data \
...
for((i=0;i<10;i+=2))
do
OUR_DIR="../checkpoint/toolkit_qwen2_name/epoch$i"
testdata="test2_seen"
deepspeed --num_gpus $num_gpus src/train.py \
    --stage sft \
    --do_predict \
...
echo $OUR_DIR
cd ../Evaluate
python evaluate_name.py $OUR_DIR $testdata
cd ../LLaMA-Factory2
done
